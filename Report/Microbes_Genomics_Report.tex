...
The tree is obtained using the following code, \code{FastTreeMP -gtr -nt -out core\_gene.tre core\_gene\_alignment.aln}, in which the parameter \code{-gtr}, the generalized time-reversible model (to be used with nucleotide alignments only), the parameter \code{-nt} is used to specify that the alignment is performed on nucleotides.
