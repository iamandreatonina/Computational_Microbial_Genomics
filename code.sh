...
-unique_vs_new_genes.png -> da commentare -> in the Rtab files, does unique gene mean genes that are only present in one strain? and new gene mean genes that are not found in previous strains?
...
Roary pangenome plots were examined to assess the relationships between numbers of genes identified and numbers of genomes used.
...
This command produce a core gene alignment, to get a better tree -> thanks to the -e parameter.
